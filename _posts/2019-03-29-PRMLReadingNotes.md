...
* 书本印刷质量：4 星。
  ...
  * 英文版排版质量更好，不过唯一可以正常途径得到的是 2006 年版，文中错误不少，一定要去下载错误修正文件将错误修正，否则部分地方影响理解。
...
* 著作翻译质量：4 星。
  ...
  * 缺点：符号的使用不如原文规范。例如：原文都以加粗的罗马字母表示向量，而译文只是以加粗默认的数字字母表示向量，而在印刷中默认的数学字母加粗和未加粗的区别不明显，容易混淆。
* 读书建议：前四章是重点，建议先从1~4读一遍，再回头1~4读一遍，然后再读后面的章节。
* 笔记目的：记录重点，方便回忆。
...
* M：表示矩阵；(_w_\_1,...,_w_\_M)：表示一个行向量有 _M_ 个元素；**w**=(_w_\_1,...,_w_\_M)^T 表示对应的列向量。
* **I**\_M：表示 _M_×_M_ 单位阵。
* _x_：表示元素；_y_(_x_)：表示函数；_f_\[_y_]：表示泛函。
* _g_(_x_)=_O_(_f_(_x_))：表示复杂度。
* E_x\[_f_(_x_,_y_)]：表示 _f_(_x_,_y_) 关于随机变量 _x_ 的期望。
...
* 重点
  * 多项式曲线拟合：这个例子通过不同角度介绍机器学习，从而更好地理解贝叶斯估计的思想，也是后面反复讨论的基础。
* 难点
...
* 训练集 (training set)：一个由 _N_ 个数字{x_1,...,x\__N_}组成的大的集合，用来通过训练来调节模型的参数。
...
* **Bayesian 概率**
  ...
  * 后验概率 ∝ 似然函数 × 先验概率
  * 似然函数
    * 似然函数：由观测数据集D来估计，可以被看成参数向量 _w_ 的函数。
...
* **高斯分布**（Gaussian distribution)，也叫正态分布 (normal distribution)。（各种分布的详细情况参考 C02）
  ...
  * 最大似然估计未知的参数μ和σ^2
    * 在给定的数据集下最大化参数的概率v.s.在给定参数下最大化数据集的概率。
...
* 平方和误差函数等价于高斯噪声模型的假设下的最大似然解。
  ...
  * 正则化最小平方
    ...
    * 正则化项 E_W(w)
      ...
      * 更加一般的正则化项
        * q=1 的情形被称为套索 (lasso)，性质为：如果λ充分大，那么某些系数会变为零，从而产生一个稀疏 (sparse) 模型。
...
* 参数分布：引入模型参数 w 的先验概率分布
  * 假定噪声精度参数β为已知常数
  * 参数 w 受精度参数α控制
  * 后验分布关于 w 的最大化等价于对平方和误差函数加上一个二次正则化项进行最小化。
