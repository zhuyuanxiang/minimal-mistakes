...
* 书本印刷质量：4 星。
  ...
  * 英文版排版质量更好，不过唯一可以正常途径得到的是 [2006 年版](https://www.microsoft.com/en-us/research/people/cmbishop/)，文中错误不少，一定要去下载错误修正文件将错误修正，否则部分地方影响理解。
  * 中文版排版有点紧凑，错误较少，并且译者已经将第一版中的错误进行了修订，然后再翻译的。但是，译文中也存在一些错误，基本不影响阅读，但是由于翻译本身就是件困难的工作，有些建议与英文版结合起来相互印证。
...
* 读书建议：
  * 前四章是重点，建议先从 1~2 读一遍，再从 1~4 读一遍，再从 1 开始读完全书。
...
* 训练集 (training set)：一个由 _N_ 个数字{$$x_1,\cdots,x_N_$$}组成的大的集合，用来通过训练来调节模型的参数。
