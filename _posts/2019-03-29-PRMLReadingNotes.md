...
* 权空间对称性
  * 因为激活函数的对称性导致权空间中解的对称性
    * 这种对称性几乎没有什么实际的用处
  * 对于M个隐含单元，任何给定的权向量都是权空间中$2^M$个等价的权向量中的一个。
  * 因为隐含单元的连接有$M!$种可能，所以每个隐含层单元的整体权空间对称性因子是$M!\times{2^M}$
  * 对于多于两层的网络，$对称性因子=\prod 每层隐含单元的对称性因子$

### S5.2. 网络训练

* 网络训练的本质就是确定参数，确定参数的本质就是基于最优化理论对误差函数求极值。
* 解决问题的关键是：输出单元的激活函数和对应的误差函数
  * 回归问题：线性输出和平方和误差函数
    * 一元目标变量：最小化误差函数等价于最大化似然函数
    * 多元目标变量：假设目标变量之间相互独立
  * 分类问题
    * 二元分类问题：Logistic Sigmoid输出和交叉熵误差函数
      * 一个二元分类问题
        * 单一的Logistic Sigmoid输出
        * 使用神经网络，神经网络有两个Softmax输出。
      * K个相互独立的二元分类问题
    * 多元分类问题：Softmax输出和多分类交叉熵误差函数
    * 交叉熵误差函数比平方和误差函数，训练速度更快，泛化能力更强。
